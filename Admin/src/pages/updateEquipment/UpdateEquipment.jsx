...
const UpdateEquipment = ({ inputs, title }) => {
  ...
  const fetchUserData = useCallback(() => {
    if (path) {
      axios(`/equipments/get/${path}`).then((data) => {
  ...
  const handlePhotoUpload = (e) => {
    ...
    if (selectedFile) {
      ...
      axios
        .post(`/uploads/photo-equipment/${path}`, formData)
        .then((response) => {
          if (response.data) {
  ...
  const handleClick = async (e) => {
    ...
    try {
      ...
      await axios.put(`/equipments/update/${path}`, updateEquipment);
      if (updateEquipment) {
